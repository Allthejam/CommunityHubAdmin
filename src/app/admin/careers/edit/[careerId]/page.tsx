...
export default function EditCareerPage() {
    ...
    const careerRef = useMemoFirebase(() => (db && careerId ? doc(db, 'careers', careerId) : null), [db, careerId]);
    const { data: careerData, isLoading: fetching } = useDoc(careerRef);
    ...
    // Absolute Ownership Priority: Guaranteed access for the platform owner
    ...
    React.useEffect(() => {
        if (careerData) {
            ...
            setDepartment(careerData.department || "");
            setReportsTo(careerData.reportsTo || "");
            setLocation(careerData.location || "Hybrid (UK Based)");
            setEmploymentType(careerData.employmentType || "Full-time");
            setSalary(careerData.salary || "");
            setStatus(careerData.status || "Draft");
            setIsInternalOnly(!!careerData.isInternalOnly);
            setDescription(careerData.description || "");
            
            if (careerData.closingDate) {
                setClosingDate(careerData.closingDate.toDate ? careerData.closingDate.toDate() : new Date(careerData.closingDate));
            }
        }
    }, [careerData]);
    ...
    const handleSave = async () => {
        ...
        if (result.success) {
            toast({ title: "Career Updated", description: "The vacancy amendments have been saved." });
            ...
            <Card className="shadow-lg border-2">
                <CardHeader className="bg-muted/20 border-b">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <CardTitle className="text-3xl font-black uppercase tracking-tighter">Edit Vacancy</CardTitle>
                            <CardDescription>Modify the specifications and status for this recruitment listing.</CardDescription>
                        </div>
                    </div>
                </CardHeader>
